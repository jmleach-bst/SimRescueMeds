#' Produce Mean Rescue Effects as Functions of Time Since Administration
#'
...
#' was administered.
#'
#' @param phi0 Numeric. See below for details on interpretation.
#' @param phi1 Numeric. See below for details on interpretation.
#' @param phi Numeric. See below for details on interpretation.
#' @param d A numeric vector of times since rescue therapy administration.
#' @param decay Character. Specify the functional form of the decay function.
#' Options are \code{"linear"}, \code{"exponential"}, \code{"gaussian"}.
#' @param delayed_peak Logical. When \code{TRUE}, uses functions modified to
...
rescue_effect_means <- function(
  ...
  if (delayed_peak == FALSE) {
    delta <- dplyr::case_when(
      decay == "linear" ~ phi0 - phi*d,
      decay == "exponential" ~ phi0*exp(-phi*d),
      decay == "gaussian" ~ phi0*exp(-phi*d^2)
    )
  } else {
    if (decay == "linear") {
      stop('No method for delayed_peak = TRUE and decay = "linear"')
    }
    delta <- dplyr::case_when(
      decay == "exponential" ~ phi0*(d^phi1)*exp(-phi*d),
      decay == "gaussian" ~ phi0*(d^phi1)*exp(-phi*d^2)
    )
  }
  return(delta)
}
